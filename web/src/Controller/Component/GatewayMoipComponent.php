...
/**
 * GatewayMoip component
 */
class GatewayMoipComponent extends Component
{
    /**
     * Default configuration.
     */
    protected $App;
    protected $responseMoip = [];
    ...
    ])
    {
        ...
        $url = Configure::read('gateways')[$ambiente][2][$parameters['type']]['url'] . '/' . $parameters['endpoint'];
        $params = json_encode($parameters['vars'], JSON_UNESCAPED_UNICODE);
        ...
        $header .= "Authorization:" . Configure::read('gateways')[$ambiente][2][$parameters['type']]['token'];
        $options = array(
            'http' => array(
                'header'  => $header,
        ...
        UtilsComponent::saveLogFile("requestMoip.log", [
            'header' => $header,
            'url' => $url,
        ...
        /**
         * A moip esta sem retorno de json em alguns casos
         */
         ...
         */
        $error = [
            0 => 'Gateway Moip indisponível, tente novamente'
        ];
        ...
        else
        {
            ...
            $this->App->mailDebug([
                'subject' => 'Apis GatewayMoip - sendRequest',
                'error' => [
                    'request' => $parameters,
                    'response_header' => $http_response_header,
                    'response' => $result
    ...
    public function createSignature($params = [])
    {
        $output = [
            'data' => [],
            'error' => [
                12 => 'Gateway com falha no modulo de assinatura, contate o administrador'
            ]
        ];

        /**
         * Converte params para metodo Moip
         */
        $parameters =  [
            'type' => 'signatures',
            'method' => 'POST',
            'endpoint' => 'subscriptions?new_customer=true',
            'vars' => [
                'code' => (string) $params['order_id'],
                'amount' => 100 * $params['total'],
                'plan' => [
                    'code' => $params['itens'][0]['product_id'],
                    'name' => $params['itens'][0]['name']
                ],
                'customer' => [
        ...
        if(isset($response['invoice']))
        {
            ...
            $invoice_status_id = $response['invoice']['status']['code'];
            $payment_status = $this->convertInvoiceStatusFirstPay($invoice_status_id);
            $invoice_payment = $this->getInvoicePayments($invoice_id);
            $invoice_payment_id = null;
            $invoice_payment_status_id = null;

            /**
             * Verifica pagamento do invoice
             */
            if($invoice_payment)
            {
                $invoice_payment_id = $invoice_payment[0]['id'];
                $invoice_payment_status_id = $invoice_payment[0]['status']['code'];
                $payment_status = $this->convertPaymentStatus($invoice_payment_status_id);
            }

            $pay_registers_db = TableRegistry::getTableLocator()->get('PayRegisters');
            $pay_data = $pay_registers_db->newEntity([
                'order_id' => $params['order_id'],
                'credit_card_id' => $params['payment']['credit_card']['id'],
                'gateway_id' => 2,
                'method_id' => 2,
                'value' => $params['total'],
                'acquirer_code' => $params['order_id'],
            ...
            if($pay_registers_db->save($pay_data))
            {
                ...
                $credit_data = $credit_card_active_db->newEntity([
                    'order_id' => $params['order_id'],
                    'gateway_id' => $params['gateway_id'],
                    'credit_card_id' => $params['payment']['credit_card']['id']
                ]);
                if(!$credit_card_active_db->save($credit_data))
                {
                    $this->App->mailDebug([
                        'subject' => 'Apis GatewayMoip - Save CliOrderCreditCardActives',
                        'error' => [
                            'request' => $parameters,
                            'response' => $response
                        ],
                 ...
                 * Atualiza order
                 */
                $this->orderSignatureStatusUpdate($params['order_id'], $payment_status);
            }
            else
            {
                $this->App->mailDebug([
                    'subject' => 'Apis GatewayMoip - Save payment',
                    'error' => [
                        'request' => $parameters,
                        'response' => $response
                    ],
        ...
        {
            $this->App->mailDebug([
                'subject' => 'Apis GatewayMoip - Generate invoice',
                'error' => [
                    'request' => $parameters,
                    'response' => $response
                ],
     ...
     * Devido a estrutura da Moip nao permitir mais de um cartao
    ...
    public function clientSignatureUpdate($params)
    {
        ...
        {
            $this->App->mailDebug([
                'subject' => 'Apis GatewayMoip - Update customer',
                'error' => [
                    'params' => $params,
                    'request' => $parameters,
                    'response' => $response
    ...
    public function clientSignatureCreditCardUpdate($params)
    {
        ...
        {
            $this->App->mailDebug([
                'subject' => 'Apis GatewayMoip - Change credit card',
                'error' => [
                    'params' => $params,
                    'request' => $parameters,
                    'response' => $response
    ...
    public function orderSignatureStatusUpdate($order_id = null, $payment_status = null)
    {
        ...
        {
            $this->App->mailDebug([
                'subject' => 'Apis GatewayMoipEvent - orderUpdate',
                'error' => [
                    'order_id' => $order_id,
    ...
    public function retrySignature($params)
    {
        ...
        {
            $this->App->mailDebug([
                'subject' => 'Apis GatewayMoip - RetrySignature',
                'error' => [
                    'params' => $params,
                    'request' => $parameters,
                    'response' => $response
    ...
    public function cancelSignature($params)
    {
        ...
        {
            $this->App->mailDebug([
                'subject' => 'Apis GatewayMoip - cancelSignature',
                'error' => [
                    'params' => $params,
                    'request' => $parameters,
                    'response' => $response
